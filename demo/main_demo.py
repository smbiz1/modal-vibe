...
This is the demo/showcase version that includes the full web UI with gallery,
homepage, and all interactive features. Perfect for demonstrating Modal Vibe
to the public or potential users.
"""
...
# Persist Sandbox application metadata in a Modal Dict
# Use separate Dict for demo to avoid conflicts with production
...
# Updated to use demo/web instead of web
image = (
    core_image
    .add_local_dir("demo/web", "/root/web")
...
# Updated to use demo/web/vite-app instead of web/vite-app
sandbox_image = (
...
async def create_sandbox_app(prompt: str) -> str:
    print(f"[DEMO] Creating sandbox app with prompt: {prompt}")

    app_directory = AppDirectory(apps_dict, app, llm_client)
    print("[DEMO] Initialized app directory")
    sandbox_app = await SandboxApp.create(app, llm_client, prompt, image=sandbox_image)
    app_directory.set_app(sandbox_app)
    print(f"[DEMO] Created image {sandbox_image.object_id}")
    print(f"[DEMO] Created and saved sandbox app with ID: {sandbox_app.id}")
...
@modal.asgi_app()  # Remove custom_domains for now - configure via Modal dashboard
def fastapi_app():
    ...
    web_app = FastAPI(
        title="Modal Vibe Demo",
        description="Demo playground for creating AI-powered sandbox applications",
        version="1.0.0-demo"
    )
    ...
    async def get_apps():
        ...
        apps_dict = await _get_apps_dict()
        print(f"[DEMO] [API /api/apps] Returning {len(apps_dict)} apps")
    ...
    async def write_app(app_id: str, request_data: WriteAppRequest):
        app = _get_app_or_raise(app_id)
        try:
            print(f"[DEMO] Starting edit for app {app_id} with text: {request_data.text[:100] if request_data.text else ''}...")
            response = await app.edit(request_data.text)
            print(f"[DEMO] Edit completed, response status: {response.status_code}")
            ...
            try:
                ...
                print(f"[DEMO] Successfully parsed response JSON: {response_data}")
            except Exception as json_error:
                print(f"[DEMO] Failed to parse JSON response: {json_error}")
            ...
            print(f"[DEMO] Error writing to relay with data: {request_data}: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    async def ping_app(app_id: str):
        ...
        try:
            print(f"[DEMO] Pinging relay at: {heartbeat_url}")
            async with httpx.AsyncClient() as client:
                response = await client.get(heartbeat_url, timeout=2.0)
                print(f"[DEMO] Ping response status: {response.status_code}")
            ...
            print(f"[DEMO] Error pinging relay: {str(e)}")
            return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    ...
    async def terminate_app(app_id: str, request_data: TerminateAppRequest):
        ...
        try:
            ...
            print(f"[DEMO] Error terminating sandbox {app_id}: {str(e)}")
            return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    ...
    async def toggle_feature_app(app_id: str, request_data: ToggleFeatureRequest):
        ...
        try:
            ...
            print(f"[DEMO] Error toggling feature status for {app_id}: {str(e)}")
            return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    ...
    async def terminate_all_sandboxes(request_data: TerminateAppRequest):
        ...
        for app_id in apps_copy:
            try:
                sandbox_app = app_directory.get_app(app_id)
                if not sandbox_app:
                    print(f"[DEMO] ❌ App {app_id} not found in catalogue")
                    continue
                success = sandbox_app.terminate()
                app_directory.remove_app(app_id)
                if success:
                    terminated_count += 1
                    print(f"[DEMO] ✅ Terminated sandbox: {app_id}")
                else:
                    failed_count += 1
                    print(f"[DEMO] ❌ Failed to terminate sandbox: {app_id}")
            except Exception as e:
                failed_count += 1
                print(f"[DEMO] ❌ Error terminating sandbox {app_id}: {str(e)}")

        for sandbox in modal.Sandbox.list(app_id=app.app_id):
            print(f"[DEMO] Sandbox: {sandbox.object_id}")
            sandbox.terminate()
...
async def clean_up_dead_apps():
    """Cleanup dead apps every minute"""
    import httpx

    print("[DEMO] Running cleanup of dead apps")
    app_directory = AppDirectory(apps_dict, app, llm_client)
